...
# 路由函数（条件边）
# ---------------------------------------------------------------------------
def route_after_agent(state: AgentState) -> Literal["permission", "cot", "final", "end"]:
    """Agent 节点后的路由决策"""
    last_msg = state["messages"][-1] if state["messages"] else None
    ...
    if iteration >= max_iter:
        if state.get("thoughts") or state.get("tool_results"):
            return "final"
        return "end"
    ...
    if state.get("need_deep_thinking") and not state.get("thoughts"):
        return "cot"
